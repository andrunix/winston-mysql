...
var mysql = function(options) {
  ...
  if(!options.fields){
    ...
    this.fields = {
      ...
      logtype:this.options.fields.logtype
    }
  }
...
//getter
transport.Mysql = mysql;

/**
 * @method log called by winston when to log somethings
 * @param info  {Object} JSON object in winston
 * @param callback {function} callback when finished
 */
mysql.prototype.log = function(info, callback) {
  "use strict";
  ...
  function openSqlPoolConnection(info){
    var pool = self.pool;
    ...
    pool.getConnection(function (err, connection) {
      ...
      //set log object
      var log = {};
      log[self.fields.level] = info.level;
      log[self.fields.message] = info.message;
      log[self.fields.meta] = JSON.stringify(info);
      log['logtype'] = self.fields.logtype;
  ...
  process.nextTick(() => openSqlPoolConnection(info));
};
